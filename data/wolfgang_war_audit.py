#!/usr/bin/env python3
"""Wolfgangknoxley WAR inventory audit -- mirror of wolfgang_pld_audit.py
pointed at WAR + the newest export. Lists every WAR-equippable owned item
grouped by slot with GSUI stats, plus flags items missing from the stats DB.
"""
import json
import re
...
EXPORT = Path(r"C:\Users\Jason\Downloads\Wolfgangknoxley 2026-06-20 22-02-31.lua")
...
OUTPUT     = Path(r"C:\Users\Jason\Desktop\Windower\addons\GSUI\data\wolfgang_war_audit.txt")
...
def main():
    ...
    excluded_non_job = []   # owned, in jobs db, but not JOB-equippable
    for name, augs in deduped:
        ...
        if 'All Jobs' not in jobs and JOB not in jobs:
            excluded_non_job.append((name, jobs))
            continue
    ...
    lines = []
    lines.append(f'Wolfgangknoxley {JOB}-equippable inventory audit')
    ...
    lines.append(f'=== OWNED BUT NOT {JOB}-EQUIPPABLE (excluded: {len(excluded_non_job)}) ===')
    for name, jobs in sorted(set((n, tuple(j)) for n, j in excluded_non_job)):
    ...
    print(f'  {sum(len(v) for v in by_slot.values())} {JOB}-equippable items across {len(by_slot)} slots.')
